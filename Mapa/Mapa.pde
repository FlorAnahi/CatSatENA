...
void setup(){
  size(1200,600);// creamos ventana de 700 por 400 pixeles
...
coordenadaX02 = width -280;
coordenadaY02 = height -115;
...
;
     ...
     perilladeGLP = cp5.addKnob ("GLP")
              .setRange(0, 10000)
              .setValue(0)
              .setPosition(coordenadaX02,coordenadaY02)
              .setRadius(50)
...
image (globo, Longitud, Latitud);
  ...
  if (valordecadena != null ) {
   ...
   //printArray(list);
    ...
    LatitudReal= float (list[14]);
    println("latitud: " + LatitudReal);
    LongitudReal= float (list[15]);
